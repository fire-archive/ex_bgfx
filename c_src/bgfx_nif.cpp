							   ...
							   const ERL_NIF_TERM argv[])
{
	try
	{
		...
		return nifpp::make(env, nifpp::str_atom("ok"));//nifpp::make(env, bgfx::init(static_cast< bgfx::RendererType::Enum >(_type), _vendor_id, _device_id, nullptr,
				//						   nullptr) ? nifpp::str_atom("ok") : nifpp::str_atom("error"));
	}
	catch (nifpp::badarg)
	{
	}
